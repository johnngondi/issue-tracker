<template>
    <default-page :title="ucwords(user.info.name) + ' Detail'"
                  content="Update Roles & Tags below."
    >
...
export default {
    ...
    methods: {
        submitForm() {
            ...
            this.statusTitle = 'Updating User';
            this.statusContent = 'Hold on while we add user.';
            this.showStatus = true;

            //
            axios.put(route('users.update', this.user.info.id), this.builder)
                .then(({status, data}) => {
                    // handle success
                    if (status === 200){
        ...
        showErrorStatusModal() {
            ...
            this.statusTitle = 'User not Updated.';
